/**
 * CATALOG DATA — the part of mockData.js that is genuinely product/catalog
 * domain data. This is the file that should shrink and eventually disappear
 * as pages move onto the real Catalog API (see src/api/catalog.js) — kept
 * separate from siteConfig.js (nav/footer) and homeData.js (homepage-only
 * presentational content) so it's obvious what maps to a future API
 * response and what doesn't.
 *
 * Every value here is still placeholder content for visual design only.
 */

/* ------------------------------------------------------------------ */
/* Try Your Logo — shared placement/colour vocabulary                  */
/* ------------------------------------------------------------------ */
...
/**
 * Stands in for what Product Listing/Detail now serve from the real Catalog
 * API for the handful of products that already exist there. Everything
 * below has not yet been recreated in the new backend (by explicit,
 * deliberate choice — see backend/README.md) and remains the homepage
 * explorer's local data source only.
 */
...
/**
 * `active: true` is a default, applied here rather than on all 33 product
 * records individually — a future record can still override it explicitly
 * (via listingMeta or the record itself) since spread order puts the
 * explicit value last. This is the "active" field the frontend audit asked
 * for, added with a minimal diff rather than touching every entry.
 */
export const listingProducts = [
  ...
  ...extraListingProducts.map((product) => ({ active: true, ...product })),
];
...
// listingMegaCategory lives in siteConfig.js — it's mega-menu nav structure
// (label -> category id), not catalog data itself.
